...
export default class Api {
  ...
  setEmail(userEmail) { this._email = userEmail; }

  request(route, method, url = this._url, body = {}) {
    return new Promise(async (resolve, reject) => {
             let options = {};
             if (this._email)
               options = {headers : {'X-User-Email' : this._email}};
             if (method === 'GET') {
  ...
  stopStack(id) { return this.request(`stack/stop/${id}`, 'GET'); }

  getStackInfo(id) { return this.request(`stack/info/${id}`, 'GET'); }

  getChain(id) { return this.request(`chain/${id}`, 'GET'); }

  listAllChains() { return this.request('chains', 'GET'); }

  listAllSnapshots(chainType = 'ganache') {
    return this.request(`snapshots/${chainType}`, 'GET');
  }

  listStacksConfigs() { return this.request('stack/list', 'GET'); }

  deleteSnapshot(id) { return this.request(`snapshot/${id}`, 'DELETE'); }

  deleteChain(id) { return this.request(`chain/${id}`, 'DELETE'); }

  downloadSnapshotUrl(id) { return `${this._url}/snapshot/${id}`; }

  reloadStackConfigs() { return this.request('stack/reload', 'GET'); }

  getDeploymentSteps() { return this.request('deployment/steps', 'GET'); }
  ...
  _parseChainUrl(data) {
    const {details : {rpc_url}} = data;
    const [, _url, _port] = rpc_url.split(':');
    return _url === '//ex-testchain.local' ? `http://localhost:${_port}`
                                           : rpc_url;
  }
}
