...
import {createDescription, randomString, setupClient} from './helpers';
...
describe.each(chainTypesToTest)(
    'Basic testchain Chains functionality for %s', async chainType => {
      const stackPayload = {
        testchain : {
          config : {
            type : chainType,
      ...
      test(`listAllChains will return empty list for email ${email}`,
           async () => {
      ...
      test('getChain will return an object with correct chain details',
           async () => {
             ...
             expect(chainDataKeys).toEqual(expect.arrayContaining([
               'id', 'title', 'node_type', 'status', 'config', 'details',
               'deployment'
             ]));

             expect(configKeys).toEqual(expect.arrayContaining([
               'accounts', 'block_mine_time', 'clean_on_stop', 'db_path',
               'deploy_ref', 'deploy_step_id', 'description', 'gas_limit', 'id',
               'network_id', 'snapshot_id', 'type'
             ]));

             expect(chainDetailsKeys).toEqual(expect.arrayContaining([
               'ws_url', 'rpc_url', 'network_id', 'id', 'gas_limit', 'coinbase',
               'accounts'
             ]));
           });

      test('listAllChains will return an array of existing chains',
           async () => {
      ...
      test('listAllChains will return an array of existing chains for email',
           async () => {
             client.api.setEmail(email);
             const {data} = await client.api.listAllChains();
      ...
      test('client will restart a stopped chain', async () => {
        ...
        const {data : {status : status2}} =
            await client.api.getChain(testchainId1);

        expect(status2).toEqual(READY);
      }, 30000);

      test('client will delete chain and it will be removed from list',
           async () => {
             ...
             await expect(client.api.getChain(testchainId1)).rejects.toEqual({
